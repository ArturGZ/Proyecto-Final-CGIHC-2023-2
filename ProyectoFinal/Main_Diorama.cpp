...
int main()
{
	...
	// Set texture units
	lightingShader.Use();
	glUniform1i(glGetUniformLocation(lightingShader.Program, "material.diffuse"), 0); //Aloja la componente difusa en la localidad 0
	glUniform1i(glGetUniformLocation(lightingShader.Program, "material.specular"), 1); //Aloja la componente especular en la localidad 1
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1f(glGetUniformLocation(lightingShader.Program, "pointLights[0].linear"), 0.025f);
		...
		glUniform1f(glGetUniformLocation(lightingShader.Program, "pointLights[1].quadratic"), 0.095f);
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "spotLight.position"), positionSpotLight.x, positionSpotLight.y, positionSpotLight.z);
		glUniform3f(glGetUniformLocation(lightingShader.Program, "spotLight.direction"), directionSpotLight.x, directionSpotLight.y, directionSpotLight.z);
		...
		model = glm::mat4(1);
		model = glm::translate(model, positionSpotLight);
		model = glm::scale(model, glm::vec3(0.2f)); // Make it a smaller cube
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glBindVertexArray(VAO);
